...
def read_clause(fname):
    ...
    for x in f:  # for each line
        if x[0] not in {'c', '%', '0'}:  # if not comment
...
def CDCL_solve(clauses, num_var):
    ...
    while not all_vars_assigned(num_var, len(M)):  # While variables remain to assign
        ...
        conflict, literal_watch = two_watch_propogate(clauses, literal_watch, clauses_literal_watched, M,
                                                      variable)  # Deduce by Unit Propogation
        ...
        while conflict != -1:
            ...
            dec_level = add_learned_clause_to(clauses, literal_watch, clauses_literal_watched, Learned_c,
                                              M)  # add Learned clause to all data structures
            Learned_count += 1
            ...
            probability, Restart_count = RandomRestart(M, back, decide_pos, probability,
                                                       Restart_count)  # Random Restart
...
def check_status(clause, M, A, B):
    ...
    if len(sym) == 0:  # if no such symbol -> Unsatisfied (conflict) clause
        ...
        return "Unresolved", M, sym[0], sym[
            1], unit  # else return two new unsatisfied variables to use for Literal_watch
...
def Backjump(M, dec_level, decide_pos,
...
def MAIN(fichier):
    ...
    #fname = input(fichier)  # Take input file name. Repeat till valid input
    print()
    #fname += ".cnf"
    fname = fichier
...
if __name__ == "__main__":
    t = timeit.Timer(stmt="man2()", globals=globals())
    print(t.timeit(100))
